...
# GLM model (without contrasts) for Lisa's data (Pyramids and palm trees) - simple combined stimulus
#
#---------------------------------------------------------------------------------------------------------------#
...
fTCD_glm_multi<-function(path,order)
{
  ...
  for(j in 1:length(filename1))
  {
    ...
    mynewfile <- paste0(getwd(),"/Chpt4_fTCD_Holly_rawdata/",strsplit(myfile, '*.exp'), '_processed.csv')
