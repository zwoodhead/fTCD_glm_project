...
#------------------------------------------------------
dir<-("/Users/zoe/Dropbox/Project A2/Doppler_Tasks/WG_SG_LG/")


filename<-'WSLG_AnalysisData_Run2.csv' # File lists all subjects and trial inclusions/exclusions
...
poiend=15 # period of interest end
...
for (mysub in 1:31)  # Can do a loop through multiple subjects, e.g. 1:31
{
  ...
  #--------------------------------------------------------
  rawdata <- rawdataRun2
  markerlist <- run2markerlist
  filelist[mysub, 63] = length(origmarkerlistRun2)
  if (runs==2){
  ...
  # Identify excluded trials from filelist
  included_trials <- filelist[mysub, 2:31] # We're just looking at the first run, i.e. the first 30 trials
  for (t in 1:30) {
    if (included_trials[t] == 0){myinclude[t] <- 0} 
  }
  
  # Deal with excluded trials
  ...
  myN[c]=length(sentgen)
    ...
    myLI[c] = mean(LRdiff[rangestart:rangeend])
    myLIeven[c]=mean(LRdiffeven[rangestart:rangeend])
    myLIodd[c]=mean(LRdiffodd[rangestart:rangeend])
    
    myside = 1
    if (myLI[c] < 0) {myside = -1}
    ...
    for (m in 1:myN[c])
    {indLI=c(indLI,mean(finalset[m,rangestart:rangeend,1]-finalset[m,rangestart:rangeend,2]))
    }
    mysd=sd(indLI)
  ...
  if(length(keepmarkers)>=8){
    ...
    csvFile2<-paste0(dir, "Rawmeans_Run2_", mysubname, "_", condition_list[c], ".csv",sep="")
