...
filename<-'WSLG_AnalysisData_Run1.csv' # File lists all subjects and trial inclusions/exclusions
...
for (mysub in 1:31)  # Can do a loop through multiple subjects, e.g. 1:31
{
  ...
  #--------------------------------------------------------
  rawdata <- rawdataRun1
  markerlist <- run1markerlist
  filelist[mysub, 63] = length(origmarkerlistRun1)
  if (runs==2){
  ...
  if(length(keepmarkers)>=8){
    ...
    csvFile2<-paste0(dir, "Rawmeans_Run1_", mysubname, "_", condition_list[c], ".csv",sep="")
